...
class BaseCAM:
    ...
    def get_cam_image(self,
        ...
        if eigen_smooth:
            cam = get_2d_projection(weighted_activations)# 这里还是调整一遍适配1D
        else:
    ...
    def forward(self, input_tensor, target_category=None, eigen_smooth=False):
        if self.cuda:
            if isinstance(input_tensor,tuple):
                # 适用于有两项或者多项输入的情况（例如：影像+临床信息）
                input_tensor = tuple([item.cuda() for item in input_tensor])
            else:
                input_tensor = input_tensor.cuda()
        if self.compute_input_gradient:
            if isinstance(input_tensor,tuple):
                # 适用于有两项或者多项输入的情况（例如：影像+临床信息）
                input_tensor = tuple([torch.autograd.Variable(item,requires_grad=True) for item in input_tensor])
            else:
    ...
    def get_target_width_height(self, input_tensor):
        if isinstance(input_tensor,tuple):
            # 适用于有两项或者多项输入的情况（例如：影像+临床信息）
            if input_tensor[0].ndim==3:
                length = input_tensor[0].size(-1)
                return length
            elif input_tensor[0].ndim==4:
    ...
    def compute_cam_per_layer(
        ...
        for target_layer, layer_activations, layer_grads in zip(self.target_layers, activations_list, grads_list):
            ...
            cam -= cam.min()
            #########################
            scaled = self.scale_cam_image(cam, target_size)# scale_cam_image要求cam恒大于0
    ...
    def scale_cam_image(self, cam, target_size=None):
        ...
        for img in cam:
            ...
            if target_size is not None:
                if img.ndim==1:
                    img = img[np.newaxis,:].repeat(len(img),axis=0)
