    ...
    if not testing:
        if training:
            if weighted:
                ...
                loader = DataLoader(split_dataset, batch_size=1, sampler=WeightedRandomSampler(weights, len(weights), replacement=True), collate_fn=collate_MIL_mtl_concat, **kwargs)
            else:
...
#     model.relocate()
    ...
    print('Init learning rate scheduler ...', end=' ')
    lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.1)#step_size=3
    ...
    for epoch in range(args.max_epoch):
        train_loop(epoch, model, train_loader, optimizer, args.n_classes, writer, 
                   loss_fn=None, multi_modal=args.multi_modal, dual_task=args.dual_task)
        stop = validate(cur, epoch, model, val_loader, args.n_classes, early_stopping, writer, 
                        loss_fn=None, results_dir=args.results_dir, multi_modal=args.multi_modal, dual_task=args.dual_task)
        if stop: 
            break
...
#         torch.save(model.state_dict(), os.path.join(args.results_dir, "s_{}_checkpoint.pt".format(cur)))
    ...
    if args.dual_task:
        _, cls_val_acc, cls_val_auc, score_val_coor, _= summary(model, val_loader, args.n_classes, multi_modal=args.multi_modal, dual_task=True)
        print( 'Validation Set, Cls acc: {:.4f}, Cls ROC AUC: {:.4f}, Score Corr: {:.4f}'.format(cls_val_acc, cls_val_auc, score_val_coor) )
        results_dict, cls_test_acc, cls_test_auc, score_test_coor, acc_loggers = summary(model, test_loader, args.n_classes, multi_modal=True, dual_task=True)
        print( 'Test Set, Cls acc: {:.4f}, Cls ROC AUC: {:.4f}, Score Corr: {:.4f}'.format(cls_test_acc, cls_test_auc, score_test_coor) )
    else:
        _, cls_val_acc, cls_val_auc, _= summary(model, val_loader, args.n_classes, multi_modal=args.multi_modal, dual_task=False)
        print( 'Validation Set, Cls acc: {:.4f}, Cls ROC AUC: {:.4f}'.format(cls_val_acc, cls_val_auc) )
        results_dict, cls_test_acc, cls_test_auc, acc_loggers = summary(model, test_loader, args.n_classes, multi_modal=args.multi_modal, dual_task=False)
...
def train_loop(epoch, model, loader, optimizer, n_classes, writer = None, loss_fn = None, multi_modal=False, dual_task=False):  
    """
    仅支持单任务。支持单模态和多模态。
    """
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu") 
    ...
    if epoch == 0:
        ...
        lambdaF = lambda iter: (iter+1)/(warmup_iters+1)
        lr_sch = lr_scheduler.LambdaLR(optimizer, lr_lambda=lambdaF)
    ...
    if n_classes == 2:
        ...
        cls_aucs = []
        binary_labels = label_binarize(cls_labels, classes=list(range(n_classes)) )
...
#     """
#     仅支持单任务。支持单模态和多模态。
#     魔改，累计多个样本的loss之后，再返向传播，达到模拟batch_size>1的效果。
#     """
#     device = torch.device("cuda" if torch.cuda.is_available() else "cpu") 
#     model.train()
#     cls_logger = Accuracy_Logger(n_classes=n_classes)
...
def validate(
    ...
    loss_fn = None, results_dir=None, multi_modal=False, dual_task=False):
...
def summary(model, loader, n_classes, multi_modal=False, dual_task=False):
    """
    仅适用于单任务.
    """
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
